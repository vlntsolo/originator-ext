// Whait for async loaders hydrate player content
let ext_timeout = setTimeout(() => exportExtend(), 10000)

function exportExtend() {

    const host = "timedtext.netflixstudios.com"
    const api_url = `https://${host}/api/request/timedText/`
    let menu = document.querySelector("div.popup")
    let itemButtons = document.getElementsByClassName("item-button")
    let dropdown = itemButtons[itemButtons.length - 1] 

    const keyButtonsArray = {
        "imarunettuCleanup": {
            label: "Restore Default View",
            func: imarunettuCleanup,
        },
        "imarunettuExportMP4": {
            label: "Export Video as .MP4",
            func: imarunettuExportMP4,
        },
        "imarunettuDoubleExportSRT": {
            label: "Export Source/Target .SRT",
            func: imarunettuDoubleExportSRT,
        },
        "imarunettuBilingualExportHTML": {
            label: "Export Bilingual .HTML",
            func: imarunettuBilingualExportHTML,
        },
        // "imarunettuImportBetterSrt": {
        //     label: "Advanced Import SRT",
        //     func: imarunettuImportBetterSrt,
        // },
        "imarunettuRunAutoQC": {
            label: "Run AutoQC",
            func: imarunettuRunAutoQC,
        }
    };
    ...
    function imarunettuPopulateDropdown() {
        ...
        dropdown = itemButtons[itemButtons.length - 1] //this should always yield the More Actions dropdown trigger
    ...
    if(eval(condition))
    {
        // console.log("Condition: " + condition + " evalued TRUE after " + firedon + "ms");
        firedon = 0;
    ...
    (function ()
    {
        ...
        var qcMeta = {}
        qcMeta.lang = LANG
        if(document.location.href.includes(host))
        {
        ...
        function setShotChanges()
        {
                ...
                host))
            {
                ...
                {
                    getJSON(
                        `https://${host}/api/request/shotchanges/clq:origination:` +
                        our_clq,
                        function (err, data)
                        {
                            if(err !== null)
    ...
    function imarunettuAddButtonsToMenu(btnid, disabled = "") {
        let gonnaBeLast = false
        if(typeof btnid == 'undefined') {
            gonnaBeLast = true
            for(btnid in keyButtonsArray) {
        ...
        {
            var btntext = keyButtonsArray[btnid].label
            var menu = document.querySelector("div.popup")
            ...
            button.onclick = function (event) {
                if(btnid == "imarunettuCleanup") {
                    event.stopImmediatePropagation();
                    imarunettuCleanup();
                }
                else {
                    const triggerAction = keyButtonsArray[btnid].func
                    triggerAction();
                    // var funktion = btnid + '();'
                    // eval(btnid + "" + "();") //Hmm, I'm gonna try storing the principal JS code of the bookmarklets as text strings properties of one big object
                }
            }
            button.append(span)
            menu.append(button)
    ...
    function imarunettuGetLang() {
        try {
            ...
            ext_timeout = setTimeout(() => exportExtend(), 4000);
        }
    }
}
